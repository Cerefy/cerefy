...
| High | **Analytics included fabricated or mislabeled executive values.** | Before the current correction, `analyticsService` derived managed budget from project count, cost savings from a token heuristic, ROI from answer counts, hard-coded positive stakeholder sentiment, and synthetic forecast labels; agent latency was always `0`. | The executive analytics surface cannot be used as a source of business truth until measurements are persisted. The service and UI are now being changed to return `null`/`—` for unsupported metrics while retaining directly measurable counts, confidence, completion, and observed execution timing. |
| High | **The default workspace route showed fabricated business operations.** | `ExecutiveMissionControl` hard-coded $2.4M ROI, 88% autonomous load, 1.2s velocity, autonomous inventory/ad-spend/security events, and fake live topology. It was the `/workspace` index route. | Route was moved to the API-backed `DashboardPage` in commit `d2679dc`; the static component is no longer reachable via the app route map. |
| Medium | **Workflow template order previously created decisions before approval.** | `WorkflowAutomationView` default order was `AI_ANALYSIS → CREATE_DECISION → APPROVAL`. | Corrected in commit `abeb675` to `AI_ANALYSIS → APPROVAL → CREATE_DECISION`; the live authenticated smoke already confirms this safe ordering works. |
| Medium | **AI execution persisted one hard-coded confidence value.** | `runtime.ts` recorded memory-agent execution history with `confidence: 55` despite no measurement. | Removed in commit `abeb675`; unknown confidence is now omitted rather than fabricated. |

| High | **`npm test` previously skipped most test suites.** | The old script ran only `src/ai/**/*.test.ts` and `src/intelligence/**/*.test.ts` (23 tests). Running all non-Postgres tests directly found 119 passing tests, including API, guardrails, queue, observability, reconstruction, and security suites. | `npm test` now runs all non-RLS test files. A separate `npm run test:rls` targets real PostgreSQL. CI now provisions PostgreSQL 16 and runs that RLS suite. Sandbox execution skips 11 RLS tests because `DATABASE_URL_TEST` is intentionally unset here; the live deployment had already been RLS-tested and CI will be the repeatable gate. |
| High | **CI did not block untested code or high/critical image vulnerabilities.** | Previous CI ran lint and build only; the Trivy action used `exit-code: '0'`. | Updated workflow runs lint, typecheck, all unit tests, PostgreSQL RLS integration, build, and fails the security job on high/critical fixable image vulnerabilities. |
