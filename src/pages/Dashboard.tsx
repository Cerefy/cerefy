            ...
            <div className="absolute inset-0 z-10 p-6 pointer-events-none flex flex-col justify-between">
              <div className="flex justify-between items-start">
                <div className="flex flex-col gap-2">
                  <div className="font-mono-data text-[12px] bg-black/60 px-3 py-1 border-l-2 border-secondary-fixed-dim">
                    CAM_01 // SEC_SECTOR_7G
                  </div>
                  <div className="font-mono-data text-[10px] text-on-surface-variant">
                    UTC 2024-05-24 14:22:01.045
                  </div>
                </div>
              ...
              <div className="absolute top-[30%] left-[25%] w-32 h-48 border border-secondary-fixed-dim bg-secondary-fixed-dim/10">
                <div className="bg-secondary-fixed-dim text-black font-mono-data text-[8px] px-1 inline-block">
                  OBJECT_HUMAN_01: 99.4%
                </div>
              </div>
              ...
              <div className="flex justify-between items-end">
                ...
                <div className="flex gap-4">
                  <button className="pointer-events-auto px-4 py-2 bg-white text-black font-label-caps text-label-caps rounded-sm hover:opacity-80 transition-all">
                    ZOOM_OPTIC
                  </button>
                  <button className="pointer-events-auto px-4 py-2 border border-white/20 text-white font-label-caps text-label-caps rounded-sm hover:bg-white/5 transition-all">
                    THERMAL_TOGGLE
                  </button>
                </div>
              </div>
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-4 font-mono-data text-[10px] text-on-surface-variant">
                <span className="flex items-center gap-1">
                  <span className="w-2 h-2 bg-secondary-fixed-dim rounded-full"></span> GPU LOAD
                </span>
                <span className="flex items-center gap-1">
                  <span className="w-2 h-2 bg-white rounded-full"></span> INF_TPS
                </span>
              </div>
            </div>
        ...
        <section className="col-span-12 xl:col-span-4 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 gap-4">
            <div className="glass-panel p-5 rounded-lg border border-white/10">
              ...
              <div className="flex items-end justify-between">
                <span className="font-mono-data text-4xl text-primary">
                  {stats?.total_tasks ?? "—"}
                </span>
                <span className="font-mono-data text-[12px] text-secondary-fixed-dim">
                  +{stats?.tasks_today ?? 0}
                </span>
              </div>
            </div>
          ...
          <div className="glass-panel rounded-lg flex-1 flex flex-col overflow-hidden border border-white/10 min-h-[400px]">
            <div className="p-4 border-b border-white/5 flex justify-between items-center bg-white/5">
              ...
              <span className="px-2 py-0.5 bg-secondary-fixed-dim text-[10px] font-mono-data rounded text-black">
                LOGS
              </span>
            </div>
              ...
              {(stats?.recent_tasks ?? []).slice(0, 5).map((task: any) => (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-mono-data text-[10px] text-secondary-fixed-dim">
                      {task.status}
                    </span>
                  ...
                  <p className="font-body-md text-sm text-primary mb-1 truncate">
                    {task.agent_role ?? "Task"}
                  </p>
                  <span className="font-label-caps text-[9px] text-on-surface-variant group-hover:text-secondary-fixed-dim">
                    VIEW_DETAILS →
                  </span>
                </div>
              ))}
              {(!stats?.recent_tasks || stats.recent_tasks.length === 0) && (
                <div className="p-8 text-center font-mono-data text-xs text-on-surface-variant">
                  NO_TASKS_DETECTED
                </div>
              )}
            </div>
