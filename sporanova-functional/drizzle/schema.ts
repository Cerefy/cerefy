...
export const memoryEntries = pgTable(
  ...
  table => [
    index("memory_workspace_type_idx").on(table.workspaceId, table.type),
    index("memory_workspace_scope_idx").on(table.workspaceId, table.scope, table.scopeId),
  ],
);
...
export const traces = pgTable(
  ...
  table => [index("traces_workspace_idx").on(table.workspaceId, table.createdAt)],
);
...
export const traceSpans = pgTable(
  ...
  {
    id: varchar("id", { length: 128 }).primaryKey(),
    traceId: varchar("traceId", { length: 128 }).notNull().references(() => traces.id, { onDelete: "cascade" }),
  ...
  table => [index("trace_spans_trace_idx").on(table.traceId)],
);
...
export const evalRuns = pgTable(
  "eval_runs",
  {
    id: varchar("id", { length: 128 }).primaryKey(),
    workspaceId: integer("workspaceId").notNull().references(() => workspaces.id, { onDelete: "cascade" }),
    testCases: jsonb("testCases").$type<unknown[]>().notNull(),
  ...
  table => [index("eval_runs_workspace_idx").on(table.workspaceId, table.createdAt)],
);
